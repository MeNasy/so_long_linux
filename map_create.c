...
int	ft_key_handler(int key_code, t_map *dt)
{
	...
	// << For linux use the if else constructs in this code sequence >>
	/*	The behaviour of your computer may be different on Linux
		so if the program does not work with this if structure,
		you can add it into the function yourself.
 		>>>> -> printf("%d\n",key_code); <<<<< to find the values corresponding
		to the w,a,s,d keys of your computer. After finding the values,
		you will need to write those values in the if condition. so that you can run the programme. */
